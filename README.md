...
**A complete test set for a secret scanner: 116 formats that must stay silent,
and 45 that must not.**

- `fp-corpus` — 116 formats, 866 lines of completely ordinary log and build
...
- `tp-corpus` — 45 formats of the places credentials actually escape from, with
  85 synthetic credentials planted in them and an answer key saying exactly which
...
| [`tp-corpus.txt.b64`](tp-corpus.txt.b64) | the true-positive half: 45 formats that DO leak, 85 planted credentials, same delimiter. Base64 — run `materialize.py` first, see below. |
...
- **`secrets`** — always empty, on all 116 sections. That is the universal claim.
- **`personal_data`** — the 57 non-secret spans a redactor may legitimately mask
...
`fpscore.py` writes the 116 sections out as files, runs whatever command you give
...
corpus: 116 sections, 866 lines, 0 credentials.
read:   94 finding(s) via built-in straw-man scanner
dropped: 0 finding(s) naming a file this corpus does not contain
control: reported -- the scanner demonstrably read these files

FALSE POSITIVES: 92, across 34 of 116 sections
personal-data matches (not counted): 2
...
- **It is a sample, not a census.** 116 formats is enough to have found
